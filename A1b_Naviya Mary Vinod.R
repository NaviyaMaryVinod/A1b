...
# Load datasets
ipl_bbb <- read_csv('C:\\Users\\naviy\\Downloads\\IPL_ball_by_ball_updated till 2024.csv',show_col_types = FALSE)
ipl_salary <- read_excel('C:\\Users\\naviy\\Downloads\\IPL SALARIES 2024.xlsx')
...
VK_runs <- runs %>% filter(Striker == "Virat Kohli") %>% pull(runs_scored)
...
# Fit the distribution to v Kohli's runs scored and get the best distribution
best_distribution <- get_best_distribution(VK_runs)
